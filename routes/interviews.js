...
router.post('/update',valid_update, async (req,res) => {
    try {
        ...
        for(const currentuser of req.body.users)
        {
            ...
            sendUpdateMail(user.email, parseInt(interview.duration.start),parseInt(interview.duration.end))
...
router.post('/',valid, async (req,res) => {
    try {
        ...
        {
            const user = await User.findById(currentuser._id)
            sendInterviewMail(user.email, parseInt(interview.duration.start),parseInt(interview.duration.end))
            user.interviews.push(interview._id)
            await user.save()
        }
        res.status(201).send()
